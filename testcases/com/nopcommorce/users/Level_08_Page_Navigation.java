...
public class Level_08_Page_Navigation extends BaseTest{
	...
	public void User_02_Switch_Page() {
		...
		downloadableProduct = customerPage.openDownloadableProductPage();
		downloadableProduct.sleepInSecond(3);
		
		// Downloadable products => Addresses
		addresses = downloadableProduct.openAddressesPage();
		addresses.sleepInSecond(3);
		
		// Addresses => Reward points
		rewardPoint = addresses.openRewardPointPage();
		rewardPoint.sleepInSecond(3);
		
		// Reward points => Customer Infor
		customerPage = rewardPoint.openCustomerPage();
		customerPage.sleepInSecond(3);
		
		// Customer Infor => Addresses
		addresses = customerPage.openAddressesPage();
		addresses.sleepInSecond(3);
		
		// Addresses => Downloadable products
		downloadableProduct = addresses.openDownloadableProductPage();
		downloadableProduct.sleepInSecond(3);		
	}
